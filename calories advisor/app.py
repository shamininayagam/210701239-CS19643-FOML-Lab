...
def get_gemini_response(input_prompt,input):
    pil_image = Image.open(io.BytesIO(image_data[0]["data"]))
    model=genai.GenerativeModel('gemini-pro-vision')
    response=model.generate_content([input_prompt,pil_image])
...
st.set_page_config(page_title="CalTrack",
                page_icon='🍚',
                layout='centered',
                initial_sidebar_state='collapsed')


st.header("CalTrack- A Calories Advisor Application")
